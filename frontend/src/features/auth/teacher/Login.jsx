    ...
    const handleLoginTeacher = async () => {
        try {
           setLoading(true);
            ...
            const res = await axios.post(`${BACKEND_URL}/teacher/auth/login`, {
                full_name,
                phone,
                password
    ...
    return (
        <div className="bg-gray-100 min-h-screen flex justify-center items-center p-3">
            <div className="bg-white w-110 rounded-xl p-4 shadow-lg">
                ...
                <button onClick={handleLoginTeacher} className="w-full bg-cyan-500 p-3 rounded-full text-white font-bold hover:bg-cyan-400 transition-colors mb-4">Login</button>
               </div>
        </div>
    )
}
